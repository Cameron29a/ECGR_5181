...
inline void CPU::Execute() {
    ...
    auto result = doTheThing(executeStage.front().checkALUop(), rs1, rs2, rs1_fp, rs2_fp);
    if (result.index() == 0) {
...
inline void CPU::printExecutedInstructions() {
    ...
    float cpi = totalCycles / totalInstructions;
    std::cout << "Total Number of Cycles: " << totalCycles << "\n";
    std::cout << "Total Number of Executed Instructions: " << totalInstructions << "\n";
    std::cout << "Cycles per Instruction (CPI): " << cpi << "\n";
    uint32_t i = 0;
